...
I built this prototype from scratch to be a detailed answer. It features a real-time JavaScript terminal for organizers and a Python/Django backend for the official Wikimedia servers.
...
7. Built a *Strict Mode* that only counts edits on the official contest list. If participant is editing random stuff just to earn points, the engine ignores it.
...
2. **`logic.py`:** Packaged as a standard **Django Management Command**. It can be added to the existing `update.py` pipeline with a single line.
   
3. Specifically for Wish #8, my regex engine `r'\|pt(-br)?'` is designed to detect both **Portuguese (pt)** and **Brazilian Portuguese (pt-br)** edits. This ensures no Lusophone editor is left behind.
   
4. I implemented a `WikidataPointRule` model and contest-level switches (`wikidata_enabled`, `wikidata_exclude_bots`, `wikidata_linked_only`) so organizers can customize the strictness of the contest from the Django Admin panel.
   
5. Once stored, points are added to the grand total via a one-line update to the `CounterHandler` method:
   `wikidata_points = sum(edit.points for edit in WikidataContribution.objects.filter(participant=user, is_portuguese=True))`
   
6. My regex `r'\|pt(-br)?'` is specifically tuned for **Wish #8**. It’s the difference between a generic tool and a community tool.
    
7. I added a `timeout=10` and `on_delete` protection. The engine is designed to be stable, resilient, and won't hang the server if the API is slow.
    
8. I have optimized the scoring logic so it can be called directly by the existing `CounterHandler.get_points()` method.
...
2. Maintainers can simply add `load_wikidata` to the existing steps list in the
   `update.py` pipeline (running alongside load_edits and load_reverts).
3. The engine will automatically detect active contests, filter out bot traffic,
   and log the scores into the `WikidataContribution` vault.
4. This setup allows the system to wake up, sync,
   and update the database in the background without affecting user experience.
...
All code, logic (PT vs GL split), and the Cache Buster fix were manually verified and implemented by me.  
I look forward to a smooth transition to the official Outreachy dashboard for my future work.
